...
def translate(sentence: str, target_lang: str = 'en', **kwargs) -> str:
    ...
    best_ans = sorted(inference(prompt=prompt, llm=seed_x_lm, **kwargs), key=lambda x: len(x[0]), reverse=True)
    logger.debug(f'ANS: {best_ans}')
    best_ans = best_ans[0]
    prompt = f'[COT]{best_ans}[COT] Translate the sentence into "{target_lang.lower()}": <sentence>{sentence}</sentence><{target_lang.lower().replace("<", "").replace(">", "")}>'
